...
import { CaptureError } from './definitions';
...
export class MediaCaptureWeb extends WebPlugin implements MediaCapturePlugin {
    ...
    return new Promise((resolve, reject) => {
      navigator.mediaDevices
        ...
        .catch((error) => {
          ...
          reject({
            code: CaptureError.CAPTURE_INTERNAL_ERR,
    ...
    return new Promise((resolve, reject) => {
      ...
      input.onchange = async (event: Event) => {
        ...
        if (!files || files.length === 0) {
          reject({
            code: CaptureError.CAPTURE_NO_MEDIA_FILES,
  ...
  async getFormatData(mediaFile: MediaFile): Promise<MediaFileData> {
